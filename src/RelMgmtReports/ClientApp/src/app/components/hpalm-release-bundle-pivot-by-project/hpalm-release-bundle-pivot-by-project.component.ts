...
import { HPALMPivotData2 } from '../../models/hpalmpivotdata2';
import { IHPALMPivotData2 } from '../../models/hpalmpivotdata2';
...
@Component({
  selector: 'hpalm-release-bundle-pivot-by-project',
  templateUrl: './hpalm-release-bundle-pivot-by-project.component.html',
  styleUrls: ['./hpalm-release-bundle-pivot-by-project.component.scss']
})

export class HpalmReleaseBundlePivotByProjectComponent implements OnInit, AfterViewInit {

  releaseList: HPALMReleaseList[];
  ddlReleaseListValue: number = 1079;
  ReleaseText: string = '';

  _pivotData : IHPALMPivotData2[];
  ...
  CreatePivotData()
  {
    ...
    this._HPALMService.getHPALMPivotData2(this.ddlReleaseListValue).subscribe(data => {
  ...
  getPivotArray(dataArray, rowIndex, colIndex, dataIndex) {
    ...
    for (var i = 0; i < dataArray.length; i++) {
      if (!result[dataArray[i].fixingTeam]) {
        result[dataArray[i].fixingTeam] = {};
      }

      if (!result[dataArray[i].fixingTeam][dataArray[i].projectTeam]) {
        result[dataArray[i].fixingTeam][dataArray[i].projectTeam] = 1;
      }
      else {
        result[dataArray[i].fixingTeam][dataArray[i].projectTeam] = result[dataArray[i].fixingTeam][dataArray[i].projectTeam] + 1;
      }
      
      //To get column names
      if (newCols.indexOf(dataArray[i].projectTeam) == -1) {
        newCols.push(dataArray[i].projectTeam);
      }
    }
